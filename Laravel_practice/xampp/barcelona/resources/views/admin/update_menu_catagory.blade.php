...
@foreach($data as $dut)
<form action="{{url('update_catagory' , $dut->id)}}" method="post" enctype="multipart/form-data">
    ...
    <div class="bg-secondary rounded h-100 p-4 w-50 mt-3">
        ...
        <div class="form-floating mb-3">
            <textarea type="text" class="form-control " style="overflow: hidden;" name="catagory_description" id="floatingInput" placeholder="name@example.com">{{ $dut->catagory_description }}</textarea>
            <label for="floatingInput">Catagory Description</label>
        </div>
        <input class="form-control bg-dark" name="catagory_img" type="file" id="formFile">
        <img src="{{asset('/asset/catagory_img/'. $dut->catagory_img)}}" width="120px" height="60px" alt="Food">
